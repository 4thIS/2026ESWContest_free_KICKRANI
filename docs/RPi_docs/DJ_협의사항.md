# DJ 협의사항 (RPi 통합 — 찬우 정리) — 🗄️ 아카이브

> ## ✅ 전 항목 종결 (2026-09-03 표기)
>
> RPi 코드가 두 트리(`RPi/pi/`·`rc_car/pi/`)로 갈려 있던 시기의 통합 협의 기록이다.
> **협의 항목은 모두 처리됐다.** 현행 상태는 [`../실행계획.md`](../실행계획.md), 설계 기준은 [`설계명세서.md`](설계명세서.md).
>
> | 항목 | 종결 |
> | --- | --- |
> | A(트리 통합)·C-2(홀센서 상수) | ✅ PR #8 (2026-07-29) |
> | C-1 4WD 전원 재계산 | ✅ [`전원계산.md`](전원계산.md) (2026-08-10) |
> | C-3 핀맵 실배선 확인 | ✅ 실기 확정 — GPIO17 손상으로 **엔코더 GPIO22 이동**, KY-003 3.3V 급전 ([`홀센서_트러블슈팅.md`](홀센서_트러블슈팅.md), 08-24) |
> | D-1 정책 수치 | ✅ 0.4 / 0.25 / 0.1 m/s (B8) · PID `kp=1.5 ki=0.8`(09-02) |
> | F-1 예비실험 / F-2 ML 담당 | ✅ 담당 **CW** — `collect_premise.py`·`analyze_surfaces.py`·`train.py` 작성 완료 (TFLite → **RF/JSON/numpy**로 변경) |
> | 앱↔Pi 통신 규격 | ✅ issue #10 종결 → [`공통계약.md`](공통계약.md) 계약 2 (RFCOMM·JSON) |
>
> 아래 본문의 "대기"·"미해결" 표기는 **작성 당시 기록**이다.
...
- **결과** → [`전원계산.md`](전원계산.md)
  ...
  - 🔗 **스톨감지는 ⑧안전장치(B2)와 겹침** → CW가 **controller**에 구현함(B6a, 2026-08-24: `safety.StallDetector`를 `controller.tick`에서 검사 → `on_error→_safe_stop`). `SpeedController`는 순수 조절기로 유지. **DJ 확인 요청** — PR #21
